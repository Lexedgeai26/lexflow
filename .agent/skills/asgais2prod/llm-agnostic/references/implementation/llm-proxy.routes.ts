...
const getGenAI = () => {
    // 1. Force strict cleanup of conflicting Google Auth variables
    // This prevents the SDK from accidentally trying to use Application Default Credentials (ADC)
    // which leads to "ACCESS_TOKEN_SCOPE_INSUFFICIENT" errors.
    ...
    // 2. Trim whitespace (common copy-paste error)
    const apiKey = rawKey.trim();

    console.log(`[LLM Proxy] Initializing Gemini with Key: ${apiKey.substring(0, 8)}... (Length: ${apiKey.length})`);

    // 3. Log all env keys for debugging (exclude values)
    const envKeys = Object.keys(process.env).filter(k => k.includes('GOOGLE') || k.includes('KEY'));
    console.log('[LLM Proxy] Active Auth Env Vars:', envKeys);
...
// User Quota Check
router.get('/quota', autoUserMiddleware, (req, res) => {
    if (!req.llmUser || !req.llmUser.quota) {
        return res.status(404).json({ error: 'Quota not found' });
    }
    res.json(req.llmUser.quota);
});
