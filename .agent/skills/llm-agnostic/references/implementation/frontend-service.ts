...
export const validateDraft = async (
  ...
  const response: any = await callLLMProxy('/generate', {
    ...
    config: {
      ...
      // thinkingConfig: { thinkingBudget: 4000 }, // Commented out until backend v1alpha support is confirmed
      // tools: [{ googleSearch: {} }]            // Commented out until backend tools support is confirmed
    }
  });
...
// Sync case to RAG Backend
export const syncCaseToRAG = async (caseData: Case) => {
  try {
    const token = localStorage.getItem('lexreply_token');
    if (!token) return;

    await fetch('http://localhost:4000/api/rag/index', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}`
    ...
    console.log(`[RAG] Synced case: ${caseData.title}`);
  } catch (err) {
    console.error(`[RAG] Failed to sync case ${caseData.id}`, err);
  }
};
