...
3.  **UI INTEGRATION**: Update existing search bars OR implement the standard **Floating Chat Widget**. All UI must be responsive and non-intrusive.
...
## 🛡️ PRE-FLIGHT CHECKS & MANDATORY CONFIG (MUST DO)

Before implementing ANYTHING, run these checks to prevent known failures:

### 1. Environment Variables & Auth
- **Check**: does `process.env` exist?
- **Golden Rule**: Implement a `bootstrap()` function in `server.ts` that validates all required keys (`GEMINI_API_KEY`, `DATABASE_URL`) before app listen.
- **Fail Fast**: If keys are missing, throw error immediately on startup.

### 2. Provider Selection (The "Hang" Fix)
- **Check**: Are you using Gemini?
- **Golden Rule**: Use `@langchain/google-genai` exclusively. Mixing providers causes hangs.
- **Timeout**: Enforce a **30s timeout** on all LLM calls using `AbortController`.

### 3. Database & Prisma
- **Check**: Is Prisma version compatible?
- **Golden Rule**: Use the centralized `src/lib/prisma.ts` pattern with `as any` type casting if experiencing IDE errors (VSCode often caches stale types).

### 4. File Watchers (The "Loop" Fix)
- **Check**: Is `nodemon` or `ts-node-dev` running?
- **Golden Rule**: You **MUST** add `--ignore vector_store` to your dev script in `package.json`.
  - `package.json`: `"dev": "nodemon --ignore vector_store ..."`

### 5. Frontend Compatibility
- **Check**: Are you using Recharts or older UI libs?
- **Golden Rule**: Install `react-is` explicitly (`npm install react-is`) to prevent Vite build failures.
...
**BEFORE STARTING, READ:** `TROUBLESHOOTING_GUIDE.md` in this skill directory.
It contains critical fixes for common issues like:
- Hanging API endpoints (Timeout fix)
- Server restart loops (Nodemon ignore)
- LangChain type incompatibilities
- React build errors

---

## 🏆 GOLD STANDARD IMPLEMENTATION RULES

These rules are non-negotiable based on past failures:

1.  **NEVER** skip the `vector_store` ignore rule.
2.  **ALWAYS** use `AbortController` for external AI API calls.
3.  **ALWAYS** filter search results by `ownerId` or `workspaceId` (Multi-tenancy safety).
4.  **ALWAYS** implement rate limiting (50 req/15min) on AI routes.
...
### Step 11: UI Integration (Floating Widget) [AUTO]
```bash
echo "🖥️ [11/11] Integrating RAG Chat Widget..."
```
If the user requests a UI (or if it's missing), implement the **Floating Chat Widget**:

1.  **Create Components** in `src/client/components/rag/`:
    *   `ChatWidget.tsx` (Floating button + window)
    *   `ChatWindow.tsx` (Message list, input, loading state)
    *   `ChatMessage.tsx` (Markdown rendering, source citations)

2.  **Integrate in App Layout**:
    *   Add `<ChatWidget />` to `App.tsx` or Main Layout.

3.  **UI Constraints**:
    *   ❌ DO NOT replace existing pages.
    *   ✅ Use fixed positioning (`fixed bottom-6 right-6`).
    *   ✅ Use Tailwind CSS for styling.
    *   ✅ Mobile responsive (`w-[350px]` on desktop, `w-full` on mobile).
